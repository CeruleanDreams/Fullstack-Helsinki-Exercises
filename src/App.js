...
const App = () => {
  ...
  let bro = persons.map( (person) => person.name === newName); //returns an array, where for each entry, it specifies true or false
  console.log(bro)

  let bruh = bro.find( (entry)  => entry === true)
  console.log(bruh) //goes through the previous array to check if any array is true; you can use .includes(true) as well
...
const updateList = () => {
  ...
  if (bruh){
    ...
    if (window.confirm("Do you want to replace?")){
      ...
      servercom.update(id, {name: newName, number: newNumber}).then(response => setPersons(persons.map( person => person.id !== id ? person : response.data )))
    }
  }
